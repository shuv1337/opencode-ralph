...
export function DialogSelect(props: DialogSelectProps) {
   ...
   * This ensures the initial selection skips category headers and disabled items.
   */
  const findFirstSelectableIndex = (): number => {
    const options = props.options.filter((opt) => props.showCategories || !opt.disabled);
  ...
  const [selectedIndex, setSelectedIndex] = createSignal(findFirstSelectableIndex());
  ...
  /**
   * Find the first selectable (non-disabled) index within the filtered options.
   * Used when resetting selection after query changes.
   */
  const findFirstSelectableFiltered = (): number => {
    const options = filteredOptions();
    for (let i = 0; i < options.length; i++) {
      if (!options[i].option.disabled) {
        return i;
      }
    }
    return 0;
  ...
  useKeyboardReliable((e: KeyEvent) => {
    ...
    if (e.name === "backspace") {
      ...
      // Reset to first selectable index after query change
      queueMicrotask(() => setSelectedIndex(findFirstSelectableFiltered()));
      return;
    }
